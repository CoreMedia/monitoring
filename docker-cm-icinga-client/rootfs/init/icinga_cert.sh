...
get_certificate() {
  ...
  [[ -f ${WORK_DIR}/pki/${HOSTNAME}/${HOSTNAME}.key ]] && return
  ...
  then
    ...
    then
      ...
      then
        ...
        [[ ! -f ${HOSTNAME}.pem ]] && cat ${HOSTNAME}.crt ${HOSTNAME}.key >> ${HOSTNAME}.pem
...
# extract vars from the ICINGA_CERT_SERVICE environment variable
#
# we check first, if the value 'true' or 'false' for realize a backward compatibility
# if the value not empty, 'true' or 'false' we verify if this a valid json and extract our needed
# variables
